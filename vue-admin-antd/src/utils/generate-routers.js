...
import { BasicLayout, BlankLayout, PageView, RouteView } from '@/components/Layouts'
...
export const generatorDynamicRouter = (menuData) => {
    return new Promise(resolve => {
        const menuNav = [], childrenNav = [];
        //后端数据,数组转树形结构 根级树数组,  根级 PID
        // listToTree(menuData, childrenNav, 0)

        //已经是树形结构
        rootRouter.children = menuData
...
const generator = (routerMap, parent) => {
    return routerMap.map(item => {
        ...
        // 是否设置了隐藏子菜单
        if (hideChildren || item.hideChildren) {
