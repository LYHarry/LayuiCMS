...
httpRequest.getPagingData = (params) => {
    return new Promise((resolve, reject) => {
        // store.dispatch('showTableLoading');
        //处理默认参数
        params = Object.assign({ method: 'POST' }, params);
        params.data = Object.assign({ pageIndex: 1, pageSize: config.pageSize }, params.data);
        httpRequest.request(params).then(res => {
            ...
            const pagingRest = {
                ...
                Pagination: {
                    ...
                    current: resData.pageIndex || 1,
                    showTotal: total => `共 ${total} 条记录`,
                    pageSize: params.data.pageSize
                }
            };
            resolve(pagingRest)
        }).catch(e => reject(e))//.finally(() => store.dispatch('hideTableLoading'))
    })
};
